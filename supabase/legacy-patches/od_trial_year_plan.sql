-- Add `trial_year` plan: one-time 1-year free trial (align with app OD_RENEWAL_PACKAGES).
...
alter table public.od_memberships add constraint od_memberships_plan_check
  check (plan is null or plan in ('month', 'year', 'hour', 'trial_year'));
...
alter table public.od_membership_renewals add constraint od_membership_renewals_plan_check
  check (plan in ('month', 'year', 'hour', 'trial_year'));
...
begin
  ...
  if p_plan is null or p_plan not in ('month', 'year', 'hour', 'trial_year') then
  ...
  elsif p_plan = 'year' or p_plan = 'trial_year' then
...
begin
  ...
  if p_plan = 'trial_year' then
    if exists (
      select 1
      from public.od_membership_renewals
      where member_id = uid and plan = 'trial_year'
    ) then
...
begin
  ...
  if sess.plan = 'month' then
    v_until := v_from + interval '1 month';
  elsif sess.plan = 'year' or sess.plan = 'trial_year' then
    v_until := v_from + interval '1 year';
  else
    v_until := v_from + interval '1 hour';
  end if;
