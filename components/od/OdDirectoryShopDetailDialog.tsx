...
import { fetchGooglePlaceDetails, googlePlacePhotoMediaUrl } from "../../lib/googlePlaceDetails";
import { getDisplayName, parseGooglePlacePhotos } from "../../lib/googlePlacePayload";
...
import { OdGooglePlaceDetailTabs } from "./OdGooglePlaceDetailTabs";
...
function normalizePlaceIdForDetailsApi(raw: string): string {
...
export const OdDirectoryShopDetailDialog: React.FC<OdDirectoryShopDetailDialogProps> = ({
  ...
  const [googlePlacePayload, setGooglePlacePayload] = React.useState<Record<string, unknown> | null>(null);
  const [googlePlaceLoading, setGooglePlaceLoading] = React.useState(false);

  React.useEffect(() => {
    if (!open) {
      setGooglePlacePayload(null);
      return;
    }
    const rawPid = shop.google_place_id?.trim();
    if (!rawPid || !googleMapsApiKey) {
      setGooglePlacePayload(null);
      setGooglePlaceLoading(false);
      return;
    }
    const placeId = normalizePlaceIdForDetailsApi(rawPid);
    let cancelled = false;
    setGooglePlaceLoading(true);
    void fetchGooglePlaceDetails(placeId, googleMapsApiKey)
      .then((payload) => {
        if (!cancelled) setGooglePlacePayload(payload);
      })
      .catch(() => {
        if (!cancelled) setGooglePlacePayload(null);
      })
      .finally(() => {
        if (!cancelled) setGooglePlaceLoading(false);
      });
    return () => {
      cancelled = true;
    };
  }, [open, shop.google_place_id, googleMapsApiKey]);

  const heroPhotoFromGoogle = React.useMemo(() => {
    if (!googlePlacePayload || !googleMapsApiKey) return null;
    const urls = parseGooglePlacePhotos(googlePlacePayload, googleMapsApiKey, 1);
    return urls[0] ?? null;
  }, [googlePlacePayload, googleMapsApiKey]);

  const headerTitle = googlePlacePayload
    ? getDisplayName(googlePlacePayload, shop.business_name)
    : shop.business_name;

  const headerRating =
    googlePlacePayload && typeof googlePlacePayload.rating === "number"
      ? googlePlacePayload.rating
      : shop.rating;
  const headerReviewCount =
    googlePlacePayload && typeof googlePlacePayload.userRatingCount === "number"
      ? googlePlacePayload.userRatingCount
      : shop.rating_count;

  const showGoogleTabs = Boolean(shop.google_place_id?.trim() && googlePlacePayload);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn(
          ...
          "md:h-[min(88vh,780px)] md:max-h-[min(88vh,780px)] md:w-[min(92vw,980px)] md:max-w-[980px] md:rounded-[1.25rem] md:data-[state=open]:animate-none md:data-[state=closed]:animate-none"
        )}
        onOpenAutoFocus={(e) => e.preventDefault()}
      >
        <motion.div
          className="flex min-h-0 flex-1 flex-col md:h-full md:max-h-none"
        ...
        >
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain [-webkit-overflow-scrolling:touch]">
            <div
              className="relative aspect-[16/10] w-full min-h-[180px] overflow-hidden sm:min-h-[220px] md:aspect-[16/7] md:min-h-[260px]"
              style={{ viewTransitionName: vt.hero } as React.CSSProperties}
            >
              {heroPhotoFromGoogle ? (
                <img
                  src={heroPhotoFromGoogle}
                  alt=""
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5">
                <DialogHeader className="space-y-2 text-left">
                  ...
                  <div className="flex items-center gap-2 text-[15px] text-[#e8eaed]">
                    ...
                    />
                    <span>
                      {headerRating != null ? headerRating.toFixed(1) : "—"}
                      {headerReviewCount != null ? ` (${headerReviewCount} reviews)` : ""}
                    </span>
                  </div>
                </DialogHeader>
              </div>
            ...
            {shop.google_place_id?.trim() && googlePlaceLoading && (
              <div className="flex justify-center py-10">
                <div className="h-9 w-9 animate-spin rounded-full border-2 border-white/20 border-t-white/80" aria-hidden />
                <span className="sr-only">Loading place details</span>
              </div>
            )}

            {showGoogleTabs && googlePlacePayload ? (
              <OdGooglePlaceDetailTabs placeDetails={googlePlacePayload} apiKey={googleMapsApiKey} shop={shop} />
            ) : null}

            {(!shop.google_place_id?.trim() || (!googlePlaceLoading && !googlePlacePayload)) && (
              <OdDirectoryFallbackSections shop={shop} place={place} />
            )}
          </div>
        </motion.div>
      </DialogContent>
...
function OdDirectoryFallbackSections({
  shop,
  place,
}: {
  shop: OdDirectoryShop;
  place: ReturnType<typeof mergeDirectoryPlaceDisplay>;
}) {
  return (
